...
const playerDiscoveryV2Repository = require('../repositories/playerDiscoveryV2Repository');
...
function registerScheduledJobs() {
  ...
  // Player Discovery V2 fraud refresh. Kept separate from V1 so the new
  // pipeline can be rolled back without changing the legacy feed.
  cron.schedule('10 * * * *', async () => {
    try {
      const { updated } = await playerDiscoveryV2Repository.recomputeFraudSignals();
      logger.info(`Discovery V2 fraud recompute: updated ${updated} player(s).`);
    } catch (err) {
      logger.error('Discovery V2 fraud recompute failed', err);
    }
  });

  logger.info(
    'Scheduled jobs registered (reconciliation every 10min, verification expiry daily 02:15, cleanup daily 03:00, V1 fraud hourly, V2 fraud hourly at :10).'
  );
}
